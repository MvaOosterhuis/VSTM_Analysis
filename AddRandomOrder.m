    ...
    for whichDT=1%:length(DTs)
        if DTs(whichDT)>0
            ...
            for addModels = 1
                ...
                if addModels == 1
                    folder=[pwd, filesep, 'Gray', filesep, 'RandomOrderAverages (L1)'];
                ...
                for whichMap=1:length(mapNames)
                    if addModels == 1
                        dataName=char(strcat(subname, '.', mapNames(whichMap), '.Left.Log2Lin.RandomOrder'));
                    else
                    end
                    ...
                    if addModels == 1
                        dataName=char(strcat(subname, '.', mapNames(whichMap), '.Right.Log2Lin.RandomOrder'));
                    else
                    end
                    %Right hemisphere
