function vw = rmRunVSTMScriptLog(vw,dataTypes, roi,wSearch,models, hrfParams, matFileName, maxCores, separateBetas)
...
if ~exist('matFileName','var') || isempty(matFileName)
    if strcmp(models{1},'1g')
        if wSearch == 10 || wSearch == 9
            matFileName = sprintf('retModel-%s-Monotonic-Log-FullBlanks-DT0.5-Fine',models{1}, datestr(now,'yyyymmdd-HHMMSS')); 
        else
        ...
        switch lower(models{n})
            case '1g'
                ...
                tmpv=rmMain([1 dataTypes(dt)],roi,wSearch,...
                    ...
                    'minrf',.03,'maxrf',1.8,'numbersigmas',400,... % sample prfs at <0.1 (whatever the unit is)
                    'spacesigmas','linear',...
                    'hrffitthreshve', 0.2,...
                    'hrf', hrfParams,...
