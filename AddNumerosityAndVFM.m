    ...
    for whichDT=1%:length(DTs)
        if DTs(whichDT)>0
            ...
            for addModels = 2%1:2
                ...
                if addModels == 1
                    folder=[pwd, filesep, 'Gray', filesep, 'Numerosity'];
                    modelFile=dir([char(strcat(folder,filesep, '*-gFit.mat'))]);
                elseif addModels == 2
                    folder=[pwd, filesep, 'Gray', filesep, 'VFM'];
                    modelFile=dir([char(strcat(folder,filesep, '*-edit.mat'))]);
                end
                ...
                for whichMap=1:length(mapNames)
                    if addModels == 1
                        dataName=char(strcat(subname, '.', mapNames(whichMap), '.Left.Numerosity.', DTnames(whichDT)));
                    elseif addModels == 2
                        dataName=char(strcat(subname, '.', mapNames(whichMap), '.Left.VFM.', DTnames(whichDT)));
                    end
                    ...
                    if addModels == 1
                        dataName=char(strcat(subname, '.', mapNames(whichMap), '.Right.Numerosity.', DTnames(whichDT)));
                    elseif addModels == 2
                        dataName=char(strcat(subname, '.', mapNames(whichMap), '.Right.VFM.', DTnames(whichDT)));
                    end
                    %Right hemisphere
